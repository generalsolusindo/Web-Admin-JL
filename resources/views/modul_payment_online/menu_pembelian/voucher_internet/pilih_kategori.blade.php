...
<body>
    ...
    <div class="mx-auto max-w-screen-lg px-4 md:px-6 py-2.5"">
        <span class="px-3 text-xl font-bold font-sans my-2">{{$vouchers['operator']}}</span>
        <div class="px-3 mt-4">
                ...
                @foreach ($vouchers['produk'] as $voucher)
                    @if ($voucher[0] === 'VOUCHER')
                        <li class="my-2 mr-1.5 lg:mr-1.5 xs:mr-2">
                            @php
                                $idKategori = $voucher[0];
                                $detailKategori = $voucher[1];
                            @endphp
                            <a href="/detailVouchers/{{$idKategori}}/{{$detailKategori}}">
                                <div
