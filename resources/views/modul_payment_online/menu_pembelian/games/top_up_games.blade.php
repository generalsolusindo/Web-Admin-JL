        ...
        <div class="px-3 my-3">
            <ul class="flex flex-wrap text-sm font-medium sm:justify-start xs:justify-around">
                @foreach ($games['produk'] as $game)
                    @if ($game[0] === 'TOPUP-GAME')
                        <li class="mr-1 my-2">
                            @php
                                $detailKategori = $game[1];
                            @endphp
                            <a href="{{ url('DetailGameTopUp', $detailKategori) }}">
                                <div
                                    class="text-center h-28 w-40 lg:w-40 md:w-40 sm:w-36 xs:w-32  border-2 border-gray-200 hover:border-orange-juallagi_gelap transition ease-in-out hover:scale-105 delay-150 duration-200">
                                    @foreach ($images as $img)
                                        @if ($img[0] === $game[1])
                                            <img src="{{ $img[1] }}" alt=""
                                                class="h-16 p-1 lg:h-16 md:h-16 sm:h-10 xs:h-10 mx-auto justify-center">
                                            <div class="py-1">
